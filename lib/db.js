...
export async function ensureSchema() {
  ...
  const { rows: hasSubmissions } = await sql`SELECT 1 FROM rsvp_submissions WHERE event_id = ${beninId} LIMIT 1`;
  if (hasSubmissions.length === 0) {
    await sql`
      INSERT INTO rsvp_submissions (event_id, nom, fonction, organisation, email, telephone, present, accompagnants, atelier, delegation_note, created_at)
      SELECT ${beninId}, nom, fonction, organisation, email, telephone, present, accompagnants, atelier, delegation_note, created_at
      FROM benin_rsvp
    `;
  }
...
export async function insertSubmission(eventId, f) {
  await ensureSchema();
  await sql`
    INSERT INTO rsvp_submissions (event_id, nom, fonction, organisation, email, telephone, present, accompagnants, atelier, delegation_note)
    VALUES (${eventId}, ${f.nom}, ${f.fonction}, ${f.organisation}, ${f.email}, ${f.telephone}, ${f.present}, ${f.accompagnants}, ${f.atelier}, ${f.delegation_note})
  `;
}
