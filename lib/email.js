...
export async function sendConfirmationEmail({ settings, nom, email }) {
  ...
  if (start) {
    ...
    calendarLinksHtml =
      '<p>Ajouter l\'événement à votre agenda :</p>' +
      '<p>' +
      ...
      '</p>' +
      '<p style="font-size:12px;color:#666;">Vous pouvez également ouvrir la pièce jointe invitation.ics pour ajouter l\'événement à Apple Calendar ou à tout autre agenda compatible.</p>';
  }
  ...
  const htmlLines = [
    ...
    settings.horaires ? `🕒 ${settings.horaires}${settings.accueil ? ` <i>(${settings.accueil})</i>` : ''}<br/>` : '',
    settings.lieu ? `📍 ${settings.lieu}<br/>` : '',
    '</p>',
    calendarLinksHtml,
    '<p>Au plaisir de vous accueillir.</p>',
    '<p>À très bientôt,</p>',
    '<p style="margin-top:16px;"><img src="https://rsvp.synelia.tech/synelia-logo.png" alt="Synelia" style="height:28px;" /></p>',
  ];
